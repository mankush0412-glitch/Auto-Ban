...
recently_ban_dm:       dict[int, int]            = {}   # {user_id: timestamp} — dedup ban DM
recently_banned_full:  dict[tuple, int]          = {}   # {(uid, gid): ts} — dedup full ban event
recent_unban_dm:       dict[tuple, int]          = {}   # {(uid, gid): ts} — dedup unban DM

# ── Global Telegram Application reference (set in lifespan) ─
...
async def keep_alive_loop():
    """Self-ping the public URL every 4 minutes so Render free service stays awake.
       Uptime monitors are great but a self-heartbeat adds an extra safety net."""
...
async def lifespan(web: FastAPI):
    ...
    # ── WEBHOOK MODE (replaces polling — survives Render restarts) ──
...
# Backwards-compat: render.yaml may reference `bot:web`
web = web_app
...
async def telegram_webhook(token: str, request: Request):
    """Telegram pushes updates here — no polling needed."""
    if token != BOT_TOKEN:
...
async def do_ban(
    ...
    # ── STRONG DEDUP: skip if same (user, group) was banned in last 60s ──
    key = (uid, gid)
    last_full = recently_banned_full.get(key, 0)
    if now_ts() - last_full < 60:
        logger.info(f"Skipped duplicate ban event: {uid} in {gid}")
        return False
    recently_banned_full[key] = now_ts()

    # ── Send ban DM — deduplicated per user (no double messages) ──
    last_dm = recently_ban_dm.get(uid, 0)
    if now_ts() - last_dm > 30:          # 30-sec dedup window
        recently_ban_dm[uid] = now_ts()
    ...
    # ── Notify GROUP OWNER (premium user) ONLY — never bot owner ──
    # If the group has no owner_id, fall back to bot owner (legacy groups).
    g_data    = get_group(gid)
    group_owner = g_data.get("owner_id") if g_data else None
    notify_id   = group_owner
    if not notify_id:
        notify_id = get_config().get("bot_owner_id") or OWNER_ID
...
async def cb_direct_unban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Allow: bot owner OR the group's own owner (premium user)
    group_owner = g.get("owner_id") if g else None
    if not is_owner(uid) and uid != group_owner:
        await q.answer("❌  Not authorised.", show_alert=True)
        return

    by_name = "Owner" if is_owner(uid) else "Group Owner"
    ok = await do_unban(context, target, gid, gname, by_name)
    if ok:
        try:
...
async def do_unban(
    ...
    # ── Use only_if_banned=True for safer unban ──
    try:
    ...
    # ── Use the saved per-group rejoin link if premium user has set one
    #    (via /setrejoinlink). Otherwise generate a fresh single-use link.
    g_data     = get_group(gid)
    invite_url = (g_data or {}).get("rejoin_link")
    ...
    if not invite_url:
        try:
            ...
            logger.warning(f"create_chat_invite_link failed for {gid}: {e}")

    # ── Dedup unban DM (avoid double messages) ──
    key = (uid, gid)
    last_dm = recent_unban_dm.get(key, 0)
    if now_ts() - last_dm > 30:
        recent_unban_dm[key] = now_ts()
        try:
            text = (
            ...
            if invite_url:
                kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton("🔙 Rejoin Group", url=invite_url)
                ]])
            await context.bot.send_message(
                uid, text,
                parse_mode="Markdown",
    ...
    # ── Clear ban-dedup so a future genuine ban can fire ──
    recently_banned_full.pop((uid, gid), None)
    return True
...
async def handle_appeal_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Send appeal to GROUP OWNER (premium user) ONLY ──
    # Only fall back to bot owner when the group has no owner registered
    # (legacy groups added before /setup). Never send to both.
    group_owner = g.get("owner_id")
    notify_id   = group_owner
    if not notify_id:
        notify_id = get_config().get("bot_owner_id") or OWNER_ID
    ...
    await update.message.reply_text(
        "✅ *Appeal Submitted!*\n"
        "_You'll be notified once reviewed._",
        parse_mode="Markdown",
    )
...
async def cb_appeal_accept(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await do_unban(context, target, gid, g["name"] if g else str(gid), "Admin")
...
async def cb_appeal_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            target,
            "❌ *Appeal Rejected*\n"
            "_Your ban remains. Contact the group admin for help._",
            parse_mode="Markdown",
        )
    except TelegramError:
        pass
...
async def cmd_addchat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await can_manage(context, chat.id, uid):
        await update.message.reply_text("❌  Admins only.")
        return
    # ── No args → show PM picker (private to admin only) ──────
    if not context.args:
        mc        = g.get("monitored_chats", {})
        ...
        buttons = [
            [InlineKeyboardButton("📋 Open Picker (Private)", url=pm_url)],
            [InlineKeyboardButton("✏️ Enter ID: /addchat -100xxx", callback_data=f"addchat_manual:{chat.id}")],
        ]
        await update.message.reply_text(
            f"➕ *Add Monitored Chat*\n"
            f"└ {g['name']}{mc_text}\n\n"
            f"_Tap below — list opens in your private chat with me, no member can see._",
            parse_mode="Markdown",
...
async def cmd_setrejoinlink(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Premium user sets a permanent invite link for their group.
    The bot will then send THIS exact link to every unbanned user
    (instead of generating a new single-use link each time).

    Usage (in the group):
        /setrejoinlink https://t.me/+abcdef...
        /setrejoinlink clear        ← remove the saved link
    """
    uid  = update.effective_user.id
    chat = update.effective_chat
    ...
    if not is_premium(uid): return
    if chat.type not in ["group", "supergroup"]:
        await update.message.reply_text("⚠️  Use this command inside the group.")
        return
    g = get_group(chat.id)
    if not g:
        await update.message.reply_text("❌  Run `/setup` first.", parse_mode="Markdown")
    ...
    if not context.args:
        current = g.get("rejoin_link")
        text = (
            "📌 *Rejoin Link Setup*\n"
            "━━━━━━━━━━━━━━━━━━━━\n\n"
            "Set a fixed invite link that the bot will\n"
            "send to every unbanned user.\n\n"
            "*Usage:*\n"
            "`/setrejoinlink <invite_url>`\n"
            "`/setrejoinlink clear` — remove saved link\n\n"
        )
        if current:
            text += f"*Current:* {current}"
        else:
            text += "_No link saved — bot will auto-generate a one-time link._"
    ...
    if arg.lower() == "clear":
        set_group(chat.id, {"rejoin_link": None})
        await update.message.reply_text(
            "✅  *Saved rejoin link removed.*\n"
            "Bot will auto-generate one-time links from now on.",
            parse_mode="Markdown",
        )
        return

    if not (arg.startswith("https://t.me/") or arg.startswith("http://t.me/")):
        await update.message.reply_text(
            "❌  Invalid link.\n"
    ...
    set_group(chat.id, {"rejoin_link": arg})
    await update.message.reply_text(
        f"✅  *Rejoin link saved!*\n\n"
        f"📌 Group: *{g['name']}*\n"
        f"🔗 Link: {arg}\n\n"
        f"_The bot will send this link to every unbanned user._",
        parse_mode="Markdown",
        disable_web_page_preview=True,
    )
...
async def cmd_transferowner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Bot-owner-only: change a group's owner_id so its appeals/ban alerts
    go to the correct premium user instead of the bot owner.

    Usage (PM the bot):  /transferowner <group_id> <new_owner_id>
    """
    uid = update.effective_user.id
    if not is_owner(uid):
    ...
    if len(context.args) < 2:
        await update.message.reply_text(
            "Usage: `/transferowner <group_id> <new_owner_id>`\n\n"
            "_Tip: use /listpremium to see premium user IDs._",
            parse_mode="Markdown",
        )
        return
    ...
    g = get_group(gid)
    if not g:
        await update.message.reply_text(f"❌  Group `{gid}` not registered.", parse_mode="Markdown")
        return

    if not is_premium(new_uid):
        await update.message.reply_text(
            f"⚠️  User `{new_uid}` is not premium yet.\n"
            f"Use `/addpremium {new_uid}` first.",
            parse_mode="Markdown",
        )
        return

    set_group(gid, {"owner_id": new_uid})
    await update.message.reply_text(
        f"✅  *Ownership transferred*\n"
        f"┌ 📌 Group: `{gid}` ({g.get('name','?')})\n"
        f"└ 👤 New owner: `{new_uid}`\n\n"
        f"All future ban alerts & appeals for this group\n"
        f"will go to the new owner.",
        parse_mode="Markdown",
    )
    ...
    try:
        await context.bot.send_message(
            new_uid,
            f"🎉 You are now the registered owner of group "
            f"*{g.get('name','?')}* (`{gid}`).\n"
            f"Ban alerts & appeals will be delivered to you.",
            parse_mode="Markdown",
        )
    except TelegramError:
        pass
...
async def cb_addchat_show(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show all available channels/groups as inline buttons directly in the group message."""
    q   = update.callback_query
    uid = q.from_user.id
    await q.answer()
    ...
    if not available:
        await q.edit_message_text(
            f"⚠️ *No chats available*\n\n"
            f"Add me as *Admin* to your channels/groups first,\n"
            f"then tap `/addchat` again.\n\n"
            f"_Or forward any message from the channel in this group._",
            parse_mode="Markdown",
        )
        return
    ...
    for c in available[:24]:
        ...
        buttons.append([InlineKeyboardButton(
            f"{icon}  {c['name']}",
            callback_data=f"addchat_pick:{gid}:{c['_id']}",
        )])
    buttons.append([InlineKeyboardButton("✖️ Cancel", callback_data=f"addchat_cancel")])
...
async def cb_addchat_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    await q.delete_message()
...
async def cb_addchat_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── Show remaining list so admin can add more ──────────
        bot_chats = get_bot_chats()
        ...
        if remaining:
            ...
            for c in remaining[:24]:
                ...
                buttons.append([InlineKeyboardButton(f"{ic}  {c['name']}", callback_data=f"addchat_pick:{gid}:{c['_id']}")])
            buttons.append([InlineKeyboardButton("✖️ Done", callback_data="addchat_cancel")])
            await q.edit_message_text(
                f"✅ *{icon} {tchat.title}* added!\n"
                f"└ {g['name']} · {len(mc)} monitored\n\n"
                f"📋 *Select another or tap Done:*",
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup(buttons),
            )
        else:
            await q.edit_message_text(
                f"✅ *{icon} {tchat.title}* added!\n"
                f"└ {g['name']} · {len(mc)} monitored\n\n"
                f"_No more chats to add. All done!_",
                parse_mode="Markdown",
            )
    except TelegramError as e:
        await q.edit_message_text(
            f"❌ Could not access chat. Make sure bot is admin there.\n`{e}`",
            parse_mode="Markdown",
        )
...
def build_bot_app() -> Application:
    ...
    # ── Owner commands  (long form + medium alias) ──────────────
    app.add_handler(CommandHandler(["addpremium",    "addprem"],  cmd_addpremium))
    app.add_handler(CommandHandler(["removepremium", "rmprem"],   cmd_removepremium))
    app.add_handler(CommandHandler(["listpremium",   "listprem"], cmd_listpremium))
    app.add_handler(CommandHandler("botstats",                    cmd_botstats))
    app.add_handler(CommandHandler("broadcast",                   cmd_broadcast))
    app.add_handler(CommandHandler(["transferowner", "transfer"], cmd_transferowner))

    # ── Premium / Group management  (long form + medium alias) ──
    app.add_handler(CommandHandler("setup",                        cmd_setup))
    app.add_handler(CommandHandler("addchat",                      cmd_addchat))
    app.add_handler(CommandHandler(["removechat",    "rmchat"],    cmd_removechat))
    app.add_handler(CommandHandler("listchats",                    cmd_listchats))
    app.add_handler(CommandHandler("whitelist",                    cmd_whitelist))
    app.add_handler(CommandHandler(["unwhitelist",   "unwlist"],   cmd_unwhitelist))
    app.add_handler(CommandHandler("setlog",                       cmd_setlog))
    app.add_handler(CommandHandler(["setrejoinlink", "setlink"],   cmd_setrejoinlink))
    app.add_handler(CommandHandler(["setschedule",   "setsched"],  cmd_setschedule))
    app.add_handler(CommandHandler(["setminmembers", "setmin"],    cmd_setminmembers))
    app.add_handler(CommandHandler("checkall",                     cmd_checkall))
    app.add_handler(CommandHandler(["unbanuser",     "unban"],     cmd_unbanuser))
    ...
    app.add_handler(CallbackQueryHandler(cb_addchat_show,   pattern=r"^addchat_show:"))
    app.add_handler(CallbackQueryHandler(cb_addchat_cancel, pattern=r"^addchat_cancel$"))
