...
async def do_ban(
    ...
    # ── Send ban DM — deduplicated (no double messages) ──────
    last_dm = recently_ban_dm.get(uid, 0)
    if now_ts() - last_dm > 15:          # 15-sec dedup window
...
async def handle_appeal_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Send appeal to GROUP OWNER (premium user), not always bot owner ──
    notify_id = g.get("owner_id") or get_config().get("bot_owner_id") or OWNER_ID
    if notify_id:
