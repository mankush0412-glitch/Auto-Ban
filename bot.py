...
async def do_ban(
    ...
    # Notify owner with direct Unban button
...
async def cmd_addpremium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            target,
            f"🎊  *Welcome to Guard Bot Premium!*\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"✨  *Your Access Details*\n"
            f"┌─ 👤 User: {uname}\n"
            f"├─ 🆔 ID: `{target}`\n"
            f"├─ 📅 Validity: *{exp_text}*\n"
            f"└─ 🗓️ Expires: *{fmt_date(expires)}*\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🛡️  *Your Premium Features*\n\n"
            f"⚡  *Auto-Ban System*\n"
            f"   Members who leave monitored channels\n"
            f"   are instantly and automatically banned.\n\n"
            f"🔔  *Owner Notifications*\n"
            f"   Get instant alerts for every ban with\n"
            f"   a one-tap *Unban* button.\n\n"
            f"📋  *Multi-Group Support*\n"
            f"   Protect unlimited groups with one bot.\n\n"
            f"📊  *Full Control Panel*\n"
            f"   Dashboard, stats, whitelist, log channel,\n"
            f"   auto-schedule — all in `/menu`.\n\n"
            f"📩  *Appeal System*\n"
            f"   Banned users can submit appeals;\n"
            f"   you approve or reject with one click.\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"🚀  *Quick Setup (3 Steps)*\n\n"
            f"1️⃣  Add {bot_username} to your group\n"
            f"    ↳ Make it *Admin* with Ban permission\n\n"
            f"2️⃣  Type `/setup` inside the group\n"
            f"    ↳ Bot will activate and confirm\n\n"
            f"3️⃣  Type `/addchat` in the group\n"
            f"    ↳ Select channels from the list *or*\n"
            f"      forward any message from the channel\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📌  *Useful Commands*\n"
            f"`/menu` — Control panel\n"
            f"`/addchat` — Add monitored channel\n"
            f"`/listchats` — View monitored chats\n"
            f"`/whitelist <id>` — Whitelist a user\n"
            f"`/checkall` — Manual ban check\n"
            f"`/mycheck` — Member status check\n"
            f"`/rules` — Show group rules\n"
            f"`/mystats` — Your premium stats\n\n"
            f"Need help? Contact the bot owner. 🙌",
            parse_mode="Markdown",
        )
    except TelegramError:
        pass
...
async def cmd_setup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"🛡️  *Auto-ban system is ACTIVE*\n\n"
        f"*Quick Setup:*\n"
        f"1️⃣  *Add channels to monitor:*\n"
        f"     • Forward any message from the channel here _(auto-detect!)_\n"
        f"     • Or type `/addchat <id>` manually\n\n"
        f"2️⃣  `/setlog <channel_id>` — Set ban log channel\n"
        f"3️⃣  `/setschedule 6` — Auto-check every 6 hours\n"
        f"4️⃣  `/menu` — Full control panel\n\n"
        f"⚡  *How it works:*\n"
        f"• New member joins → bot checks all required channels\n"
        f"• Member leaves any required channel → *instant ban* 🚫\n"
        f"• Banned user gets a DM explaining the reason\n"
        f"• Owner gets notified with a direct *Unban* button",
        parse_mode="Markdown",
    )
...
async def cmd_addchat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await can_manage(context, chat.id, uid):
        ...
        return
    # ── Show inline picker if no args ─────────────────────────
    if not context.args:
        mc         = g.get("monitored_chats", {})
        bot_chats  = get_bot_chats()
        available  = [c for c in bot_chats if str(c["_id"]) not in mc and c["_id"] != chat.id]

        buttons = []
        for c in available[:20]:   # max 20 buttons
            icon = "📢" if c.get("type") == "channel" else "👥"
            buttons.append([InlineKeyboardButton(
                f"{icon}  {c['name']}",
                callback_data=f"addchat_pick:{chat.id}:{c['_id']}",
            )])

        # Manual entry + forward hint
        buttons.append([InlineKeyboardButton("✏️  Enter ID Manually", callback_data=f"addchat_manual:{chat.id}")])

        kb    = InlineKeyboardMarkup(buttons)
        count = len(available)
        hint  = (
            f"🔍  *{count} chat(s) available* where I'm admin.\n"
            f"Select one below, or forward any message from a channel here."
            if count else
            "⚠️  *No chats found yet.*\n\n"
            "Add the bot as admin to your channels/groups first,\n"
            "then forward any message from the channel here OR\n"
            "use `/addchat <chat_id>` manually."
        )
        ...
        if mc:
            ...
            for info in mc.values():
                ...
                mc_lines.append(f"  {icon}  {info['name']}")
        ...
        await update.message.reply_text(
            ...
            f"{''.join(mc_lines)}",
            parse_mode="Markdown",
            reply_markup=kb if buttons else None,
        )
        return
...
async def cb_addchat_manual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin clicked 'Enter ID Manually'."""
    q   = update.callback_query
    await q.answer()
    gid = int(q.data.split(":")[1])
    await q.edit_message_text(
        f"✏️  *Enter Chat ID Manually*\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"Send the command with the chat ID:\n"
        f"`/addchat -100xxxxxxxxxx`\n\n"
        f"💡  *How to get a chat ID:*\n"
        f"• Forward any message from the channel to @userinfobot\n"
        f"• Or use @username\\_to\\_id\\_bot\n"
        f"• Channel IDs usually start with `-100`",
        parse_mode="Markdown",
    )
...
def build_bot_app() -> Application:
    ...
    # Inline callbacks — addchat picker
