# 🛡️ Guard Bot — Professional Edition v2.0
...
rules - View required chats for this group
setup - Activate bot in your group (Premium)
addchat - Add a monitored chat (Premium)
removechat - Remove a monitored chat (Premium)
listchats - View all monitored chats (Premium)
whitelist - Whitelist a user from auto-ban (Premium)
unwhitelist - Remove user from whitelist (Premium)
setlog - Set a log channel (Premium)
setschedule - Set auto-check interval (Premium)
setminmembers - Set member count alert (Premium)
checkall - Manually check and ban all members (Premium)
unbanuser - Unban a user (Premium)
mystats - View your group statistics (Premium)
addpremium - Grant premium to a user (Owner)
removepremium - Revoke premium from a user (Owner)
listpremium - List all premium users (Owner)
botstats - Full bot statistics (Owner)
broadcast - Send message to all premium users (Owner)
```
...
| Command | Description |
|---------|-------------|
| `/addpremium <id> [days]` | Grant premium (lifetime or N days) |
| `/removepremium <id>` | Revoke premium |
| `/listpremium` | List all premium users |
| `/botstats` | Full statistics |
| `/broadcast <msg>` | Message all premium users |
...
| `/addchat <id>` | Add a monitored chat |
| `/removechat <id>` | Remove a monitored chat |
| `/listchats` | List monitored chats |
| `/whitelist <id>` | Exempt user from auto-ban |
| `/unwhitelist <id>` | Remove from whitelist |
| `/setlog <id>` | Set log channel |
| `/setschedule <hours>` | Set auto-check interval |
| `/setminmembers <num>` | Set member count alert |
